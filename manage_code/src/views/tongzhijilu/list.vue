	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('tongzhijilu','新增')">
					...
					<el-button class="edit_btn" type="warning" :disabled="selRows.length?false:true" @click="batchRetryClick" v-if="btnAuth('tongzhijilu','批量重试')">
						<i class="iconfont icon-xiugai5"></i>
						批量重试
					</el-button>
					<el-button class="view_btn" type="info" @click="statisticsClick">
			...
			<el-table
				...
				<el-table-column min-width="140"
					...
					align="left"
					header-align="left"
					prop="tongzhileixingmingcheng"
					label="通知类型">
					<template #default="scope">
						<el-tag v-if="scope.row.tongzhileixing == 1" type="success">{{scope.row.tongzhileixingmingcheng}}</el-tag>
						<el-tag v-else-if="scope.row.tongzhileixing == 2" type="warning">{{scope.row.tongzhileixingmingcheng}}</el-tag>
						<el-tag v-else-if="scope.row.tongzhileixing == 3" type="danger">{{scope.row.tongzhileixingmingcheng}}</el-tag>
						<span v-else>{{scope.row.tongzhileixingmingcheng}}</span>
					</template>
				</el-table-column>
				<el-table-column min-width="140"
					:resizable='true'
				...
				<el-table-column min-width="120"
					...
					<template #default="scope">
						...
						<el-tag v-else-if="scope.row.jieshouzhuangtai == 1" type="success">已接收</el-tag>
				...
				<el-table-column label="操作" width="250" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id)" v-if=" btnAuth('tongzhijilu','修改')">
							<i class="iconfont icon-xiugai5"></i>
							修改
						</el-button>
						...
						<el-button class="edit_btn" v-if="scope.row.fasongzhuangtai == 2 && btnAuth('tongzhijilu','重试')" type="warning" @click="retryClick(scope.row.id)">
							<i class="iconfont icon-xiugai5"></i>
							重试
						</el-button>
					</template>
		...
		<!-- 统计弹窗 -->
		<el-dialog title="通知发送统计" v-model="statisticsVisible" width="500px">
			<el-descriptions :column="1" border>
				...
				<el-descriptions-item label="发送失败">{{statistics.failedCount}}</el-descriptions-item>
				<el-descriptions-item label="待发送">{{statistics.pendingCount}}</el-descriptions-item>
				<el-descriptions-item label="已接收">{{statistics.receivedCount}}</el-descriptions-item>
			</el-descriptions>
		</el-dialog>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.jieshouzhuangtai!==undefined&&searchQuery.value.jieshouzhuangtai!==''){
	...
	const batchRetryClick = () => {
		...
		if (!ids.length) {
			context?.$toolUtil.message('选中的记录中没有发送失败的', 'error')
			return
		}
